...
import { Keypair, Operation, SorobanDataBuilder, SorobanRpc, TimeoutInfinite, Transaction, TransactionBuilder, xdr, } from '@stellar/stellar-sdk';
import { config } from './env_config.js';
import { getLedgerKeyWasmId } from './contract.js';
/**
...
export async function sendTransaction(transaction, parser) {
    try {
        ...
        if (get_tx_response.status === 'SUCCESS') {
            ...
            console.log('Transaction successfully submitted with hash:', send_tx_response.hash, 'Result:', result);
            return result;
        }
        else {
 ...
 */
export async function simulateRestorationIfNeeded(wasmHash, txParams) {
    try {
        ...
        if (response.entries && response.entries.length > 0 && response.entries[0].liveUntilLedgerSeq) {
            ...
            const restorationSimulation = await config.rpc.simulateTransaction(restoreTx);
            ...
            if (restoreNeeded) {
                ...
                const bumpTTLtx = new TransactionBuilder(account1, txParams.txBuilderOptions)
                    ...
                    .build();
                const ttlSimResponse = await config.rpc.simulateTransaction(bumpTTLtx);
                const assembledTx = SorobanRpc.assembleTransaction(bumpTTLtx, ttlSimResponse).build();
                const signedTx = new Transaction(await txParams.signerFunction(assembledTx.toXDR()), config.passphrase);
 ...
 */
export async function handleRestoration(simResponse, txParams) {
    ...
    const simulation = await config.rpc.simulateTransaction(restoreTx);
 ...
 */
export async function invokeSorobanOperation(operation, parser, txParams, sorobanData) {
    ...
    const isSignedCorrectly = signedTx.signatures.some((signature) => sourceKeypair.verify(txHash, signature.signature()));
 ...
 * @param {TxParams} txParams - Transaction parameters.
 */
export async function invokeClassicOp(operation, txParams) {
    const account = await config.rpc.getAccount(txParams.account.accountId());
    const txBuilder = new TransactionBuilder(account, txParams.txBuilderOptions)
        .addOperation(xdr.Operation.fromXDR(operation, 'base64'))
        .setTimeout(TimeoutInfinite);
    const transaction = txBuilder.build();
    const signedTx = new Transaction(await txParams.signerFunction(transaction.toXDR()), config.passphrase);
    console.log(`Submitting classic operation with transaction hash: ${signedTx.hash().toString('hex')}`);
    try {
