import { Address, Operation, StrKey, hash, xdr, } from '@stellar/stellar-sdk';
import { randomBytes } from 'crypto';
import { readFileSync } from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';
import { config } from './env_config.js';
import { invokeSorobanOperation } from './tx.js';
 ...
 */
export function getLedgerKeyWasmId(wasmHash) {
 ...
 */
export async function getContractCodeLedgerEntry(wasmHash
 ...
 */
export async function installContract(wasmKey, txParams, addressBook) {
    const contractWasm = readFileSync(path.join(__dirname, CONTRACT_REL_PATH[wasmKey]));
    const wasmHash = hash(contractWasm);
 ...
 */
export async function deployContract(contractKey, wasmKey, txParams, addressBook) {
 ...
 * @param {TxParams} txParams - Transaction parameters.
 */
export async function bumpContractInstance(contractKey, txParams, addressBook) {
    ...
    await invokeSorobanOperation(Operation.extendFootprintTtl({ extendTo: 535670 }).toXDR('base64'), () => undefined, txParams, bumpTransactionData);
}
/**
 ...
 * @param {TxParams} txParams - Transaction parameters.
 */
export async function bumpContractCode(wasmKey, txParams, addressBook) {
    const wasmHash = Buffer.from(addressBook.getWasmHash(wasmKey), 'hex');
 ...
 * @param {TxParams} txParams - Transaction parameters.
 */
export async function bumpContractData(contractKey, dataKey, txParams, addressBook) {
    const address = Address.fromString(addressBook.getContractId(contractKey));
    const contractDataXDR = xdr.LedgerKey.contractData(new xdr.LedgerKeyContractData({
        contract: address.toScAddress(),
        key: dataKey,
        durability: xdr.ContractDataDurability.persistent(),
    }));
 ...
 * @param {TxParams} txParams - Transaction parameters.
 */
export async function restoreContractData(contractKey, dataKey, txParams, addressBook) {
    const address = Address.fromString(addressBook.getContractId(contractKey));
    const contractDataXDR = xdr.LedgerKey.contractData(new xdr.LedgerKeyContractData({
        contract: address.toScAddress(),
        key: dataKey,
    ...
    await invokeSorobanOperation(Operation.restoreFootprint({}).toXDR('base64'), () => undefined, txParams, restoreTransactionData);
}
/**
