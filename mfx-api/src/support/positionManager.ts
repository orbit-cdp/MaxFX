...
export class PositionManagerContract extends Contract {
  ...
  static readonly parsers = {
    initialize: (result: string) => PositionManagerContract.spec.funcResToNative('initialize', result),
    pool_open_position: (result: string) => PositionManagerContract.spec.funcResToNative('pool_open_position', result),
    set_admin: (result: string) => PositionManagerContract.spec.funcResToNative('set_admin', result),
    set_fee_taker: (result: string) => PositionManagerContract.spec.funcResToNative('set_fee_taker', result),
    get_fee_taker: (result: string) => PositionManagerContract.spec.funcResToNative('get_fee_taker', result),
    get_pool: (result: string) => PositionManagerContract.spec.funcResToNative('get_pool', result),
    get_amm: (result: string) => PositionManagerContract.spec.funcResToNative('get_amm', result),
  };
   ...
   */
  initialize(admin: string, fee_taker: string, blend_pool: string, amm: string): string {
   ...
   * @returns {string} The transaction ID in base64 encoding.
   */
  pool_open_position(user: string, lend: string, borrow: string, amount: i128, amount2: i128): string {
   ...
   * @returns {string} The transaction ID in base64 encoding.
   */
  set_admin(new_admin: string): string {
    const invokeArgs = {
      method: 'set_admin',
   ...
   * @returns {string} The transaction ID in base64 encoding.
   */
  set_fee_taker(fee_taker: string): string {
    const invokeArgs = {
