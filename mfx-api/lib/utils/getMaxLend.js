...
export const getReserveConfig = (assetName) => {
...
export function calculateNextAmount(currentAmount) {
...
export function calculateMaxBorrowAmount(userPoolData, reserve) {
    ...
    const to_bounded_hf = (userPoolData.positionEstimates.totalEffectiveCollateral -
        userPoolData.positionEstimates.totalEffectiveLiabilities * 1.02) /
        1.02;
    // Determine the maximum borrowable amount
    const maxBorrowAmount = Math.min(to_bounded_hf / (assetToBase * liabilityFactor), reserve.estimates.supplied * (reserve.config.max_util / 1e7 - 0.01) - reserve.estimates.borrowed);
